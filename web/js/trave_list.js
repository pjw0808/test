...
$(document).ready(function(){
    ...
    `<div class="item">
        <div class="img_box">
            <img src="${TRAVEL_LIST[cast][0].src}" alt="">
        </div>
        <div class="desc_box">
            <div class="title">${TRAVEL_LIST[cast][0].title}</div>
            <div class="airplane">${TRAVEL_LIST[cast][0].airplane}</div>
            <div class="where">${TRAVEL_LIST[cast][0].where}</div>
            <div class="price">${TRAVEL_LIST[cast][0].price}</div>
        </div>
    // </div>`
    // let rand = Math.random()
    //          let land = Math.floor(rand*TRAVEL_LIST.length+1)
    //        let country =TRAVEL_LIST[land].length
    //        console.log(country)
    //        console.log(TRAVEL_LIST[land][Math.floor(rand*country+1)])
    // console.log(TRAVEL_LIST.length)
    let rand_arr = new Array(0);
    console.log("여기: ", rand_arr, rand_arr.length)
    let count = 0
    while(count < 9) {
        ...
        if(count == 0) {
            rand_arr.push(rand);
            // console.log("if: ", rand_arr)
            count+=1;
        }
        else {
            let chk_match = false;
            for(let i=0; i< rand_arr.length; i++) {
                // console.log(rand_arr[i] , rand)
                if(rand_arr[i] == rand) {
                    chk_match=true;
                    break;
                }
    ...
    for(let i=0; i < TRAVEL_LIST.length; i++){
        let rand = Math.random()
        let land = Math.floor(rand*TRAVEL_LIST.length)
        let country =TRAVEL_LIST[land].length
        console.log(country)
        ...
        `<div class="item">
        <div class="img_box">
            <img src="${TRAVEL_LIST[land][Math.floor(rand*country)].src}" alt="" style="with:454.44px; height:300px">
        </div>
        <div class="desc_box">
            <div class="title">${TRAVEL_LIST[land][Math.floor(rand*country)].title}</div>
            <div class="airplane">${TRAVEL_LIST[land][Math.floor(rand*country)].airplane}</div>
            <div class="where">${TRAVEL_LIST[land][Math.floor(rand*country)].where}</div>
            <div class="price">${TRAVEL_LIST[land][Math.floor(rand*country)].price}</div>
        </div>
    </div>`
    $('.recomendation').append(best)
    console.log(TRAVEL_LIST[land][Math.floor(rand*country)])
}
})